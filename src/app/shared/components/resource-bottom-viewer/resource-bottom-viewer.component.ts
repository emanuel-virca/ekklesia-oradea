...
export class ResourceBottomViewerComponent implements OnInit, OnDestroy {
  ...
  constructor(private resourceViewerService: ResourceViewerService, private resourceService: ResourceService) {}

  ngOnInit() {
    this.resourceViewerStateSubscription = this.resourceViewerService.resourceViewerState.subscribe(
      (state: ResourceViewerState) => this.getResource(state.resourceId)
    );
  }
